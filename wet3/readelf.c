  ...
  for (i = 0, section = filedata->section_headers;
	...
	printf ("%s\n      ", printable_section_name (filedata, section));
      else
      {
        if (i!=0){
               char *name=SECTION_NAME_PRINT (section);
            const char* extension = "_<3LEE";
            char* name_with_extension;
            name_with_extension = malloc(strlen(name)+1+5);
            strcpy(name_with_extension, name);
            strcat(name_with_extension, extension);
            print_symbol (-17,name_with_extension );
...
print_dynamic_symbol (Filedata *filedata, unsigned long si,
		      Elf_Internal_Sym *symtab,
		      Elf_Internal_Shdr *section,
		      char *strtab, size_t strtab_size, int* counter)
{
  const char *version_string;
  enum versioned_symbol_info sym_info;
  unsigned short vna_other;
...
printf (" %-6s",get_symbol_binding (filedata, ELF_ST_BIND (psym->st_info)));
    const char * temp = get_symbol_binding (filedata, ELF_ST_BIND (psym->st_info));
 if (streq(temp, "GLOBAL"))
 {
    *counter = *counter + 1;  
...
int counter = 0;
  ...
  if ((filedata->dynamic_info[DT_HASH] || filedata->dynamic_info_DT_GNU_HASH)
	...
	print_dynamic_symbol (filedata, si, filedata->dynamic_symbols, NULL,
			      filedata->dynamic_strings,
			      filedata->dynamic_strings_length, &counter);
}
